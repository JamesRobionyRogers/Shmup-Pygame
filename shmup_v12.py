# Pygame Shmup Part 12 & 13:  Power Ups
...
HEIGHT = 600
FPS = 60
POWERUP_TIME = 5000  # 5000 milisecounds == 5 secounds
...
pygame.display.set_caption("Shmup!  version 12 & 13")  # title of the game window
...
class Pow(pygame.sprite.Sprite):  # Power Ups
...
shield_up_sound = pygame.mixer.Sound(path.join(snd_dir, 'powerup_shield_up.wav'))
shield_down_sound = pygame.mixer.Sound(path.join(snd_dir, 'powerup_shield_down.wav'))
power_sound = pygame.mixer.Sound(path.join(snd_dir, 'powerup_gun.wav'))
expl_sounds = []
...
# Update / Checks
    ...
    for hit in hits:
        ...
        if random.random() < 0.1:  # spawn rate of powerups (10% chance )
...
# - Make the explosion sound effects quieter using audacity
